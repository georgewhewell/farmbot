...
pub struct Farmbot {
    ...
    i2cbus: &'static BusManager<Mutex<I2cdev>>,
...
impl Farmbot {
    pub fn new(config: &FarmbotConfig) -> Result<Farmbot> {
        ...
        let bus: &'static _ = shared_bus::new_std!(I2cdev = i2c_bus).unwrap();
    ...
    fn farm(&mut self) -> Result<()> {
        ...
        if let Some(ref mut tank) = self.tank {
            if tank.get() == false {
                warn!("Tank level is too low, skip farming");
                return Ok(());
            }
        } else {
    ...
    pub fn init_sensors(&mut self) -> Result<()> {
        if let Ok(tank) = tank::TankLevelSensor::new(&self.config) {
            info!("Create TankSensor: {:?}", tank);
            self.send_mqtt(MQTTPublish::BinarySensorConfig {
                topic: "homeassistant/binary_sensor/farmbot/water_tank/config".to_string(),
    ...
    pub fn start_mqtt(&mut self) -> Result<()> {
        info!("Starting MQTT");
        let mqtt = MQTTConnection::new(&self.config)?;
        self.mqtt = Some(mqtt);
        Ok(())
    }
    ...
    fn read_sensors(&mut self) -> Result<()> {
        ...
        if let Some(ref mut tank) = self.tank {
            if tank.should_read() {
                datas.push(MQTTPublish::DeviceState {
                    topic: "homeassistant/binary_sensor/farmbot/water_tank/state".to_string(),
