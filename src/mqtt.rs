...
pub enum MQTTPublish {
    ...
    BinarySensorConfig {
        topic: String,
        payload: MQTTBinarySensorConfig,
    },
    SwitchConfig {
        topic: String,
...
#[derive(Serialize, Clone)]
pub struct MQTTBinarySensorConfig {
    pub name: String,
    pub unique_id: String,
    pub device: MQTTDevice,
    pub state_topic: String,
...
impl MQTTConnection {
    ...
    pub fn new(config: &FarmbotConfig) -> Result<Self> {
        let mut mqttoptions = MqttOptions::new("farmbot", &config.mqtt_hostname, 1883);
        ...
        thread::spawn(move || {
            ...
            for notification in pub_receiver.iter() {
                match notification {
                    ...
                    MQTTPublish::BinarySensorConfig { topic, payload } => {
                        client
                            .publish(
                                topic,
                                QoS::AtLeastOnce,
